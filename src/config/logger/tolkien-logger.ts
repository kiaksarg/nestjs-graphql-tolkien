...
/**
 * @description
 * The TolkienLogger interface defines the shape of a logger service which may be provided in
 * the config.
 *
 * @docsCategory Logger
 */
export interface TolkienLogger {
...
const noopLogger: TolkienLogger = {
  error() {
    /* */
  },
  warn() {
...
/**
 * @description
 * The Logger is responsible for all logging in a Tolkien application.
 *
 * It is intended to be used as a static class:
 *
 * @example
 * ```ts
 * import { Logger } from '\@tolkien/core';
 *
 * Logger.info(`Some log message`, 'My Tolkien Plugin');
 * ```
 *
 * The actual implementation - where the logs are written to - is defined by the {@link TolkienLogger}
 * instance configured in the {@link TolkienConfig}. By default, the {@link DefaultLogger} is used, which
 * logs to the console.
 *
 * ## Implementing a custom logger
 *
 * A custom logger can be passed to the `logger` config option by creating a class which implements the
 * {@link TolkienLogger} interface. For example, here is how you might go about implementing a logger which
 * logs to a file:
 *
 * @example
 * ```ts
 * import { TolkienLogger } from '\@tolkien/core';
 * import fs from 'fs';
 *
 * // A simple custom logger which writes all logs to a file.
 * export class SimpleFileLogger implements TolkienLogger {
...
export class Logger implements LoggerService {
  ...
  private static _logger: TolkienLogger;

  static get logger(): TolkienLogger {
  ...
  /** @internal */
  static useLogger(logger: TolkienLogger) {
